...
- Если продавцу не доставлено уведомление о снятии объявлений, владельцу
  (ADMIN_ID/ADMIN_TELEGRAM_IDS, зарегистрированному в users) и администраторам
  создаётся отдельное уведомление: имя, Telegram ID, причина недоставки.
  Только явный Telegram 403 «bot was blocked by the user» называется блокировкой.
  Таймаут, другой 403 и сетевой сбой не выдаются за доказательство блокировки.
  Маркер admin_delivery_alert_queued в исходном уведомлении устанавливается
  атомарно с записью оповещения, поэтому restart не создаёт его повторно.
  Автоматического бана пользователя в AutoFlow нет.
  Формат ошибок: [Telegram Bot API](https://core.telegram.org/bots/api#making-requests).

## Неактивность и деньги

В оплаченной сделке последнее новое сообщение/изменение данных передачи покупателя
...
По уточнению владельца обычная переписка не вызывает снятия объявлений.
Для торга без покупки срок считается от created_at конкретного PriceOffer.
Если продавец за 24 часа не принял и не отклонил предложение, оно получает
cancelled, а ветка закрывается, если в ней не осталось других активных предложений.
Покупателю сохраняется уведомление об отмене. Объявления остаются active,
списания/возврата нет. Обычное сообщение продавца и новое предложение не
продлевают срок старого предложения. Принятые/отклонённые предложения не отменяются.
Сервер не разрешает принять просроченное предложение до ближайшего запуска worker.
...
Автоматический возврат разрешён только
...
форме. Снятие с публикации не означает физическое удаление истории или бан.
...
- Существующий run_seller_response_timeout_worker дополнен истечением PriceOffer.
...
- Backend: полный pytest, 245 passed.
...
- SQLite service/HTTP tests: unread 1→2→0, duplicate client request,
  ...
  обычный диалог без санкций, отмена торга без снятия объявления, refund один раз,
  сохранение истории, закрытие ветки, точное распознавание блокировки бота,
  однократное уведомление администратора о недоставке без бана продавца.
