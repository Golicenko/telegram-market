...
SELLER_PAYOUT_PERCENT=100
TRAINING_SELLER_PAYOUT_PERCENT=70
PAYOUT_NFT_THRESHOLD_STARS=350
STAR_TOPUP_MIN=100
STAR_TOPUP_MAX=1000
...
- Каталог, обучение, профиль, реклама и уведомления загружаются независимо через `Promise.allSettled`. Ошибка одного раздела не скрывает интерфейс и не ломает навигацию.
...
| `favorites`, `cart_items` | избранное и исторические записи отключённой пользовательской корзины; таблицы сохранены без потери данных |
...
- Устаревшие `GET /api/cart`, `POST|DELETE /api/cart/items/{listing_id}` и `POST /api/cart/checkout` помечены deprecated и временно сохранены только для совместимости с историческими данными и ранее выставленными `cart_checkout`-счетами. Frontend их не вызывает.
...
- Пользовательская корзина удалена из интерфейса: покупка запускается напрямую из карточки или полной страницы автомобиля, а доступность и баланс повторно проверяются backend непосредственно перед покупкой.
...
- Продажа автомобиля: 100% продавцу, комиссия продажи 0%. Старые завершённые сделки сохраняют исторические суммы. Комиссия 30% действует только при выводе подарками; расчёты обучения настраиваются отдельно через TRAINING_SELLER_PAYOUT_PERCENT.
